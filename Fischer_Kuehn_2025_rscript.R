...
## last update: May 2025 (d.fischer@uke.de; Djo Fischer)
...
load('Fischer_Kuehn_2025_data.Rdata')
...
Daten$X <- Daten$CST_ACCSC

ggplot(data = Daten[Daten$CST_ACCSC_isoutlier == 0, ], aes(x = Exposition, y = X, fill=Exposition)) +
  geom_violinhalf(flip = c(1), color = "transparent") +
  stat_summary(fun.data = "mean_cl_boot", geom = "pointrange", 
               colour = "black") +
  geom_line(data = Daten[Daten$CST_ACCSC_isoutlier == 0, ], aes(x = Exposition, y = X, group=ID), na.rm = TRUE, colour="grey") +
  labs(y="ZVT", x= "Exposition" )+ 
...
Bay <- stan_glmer(ZVT ~ Exposition + TZP + (1 | ID), 
                data=all[all$ZVT_isoutlier == 0  & all$ST_Hyposmie == 0 & 
                all$Baum == "Hinoki" , ] ,
...
pvaluesH1 <- c(0.75, 0.43, 0.25, 0.17, 0.25, 0.11, 0.61, 0.02,
               0.38, 0.58, 0.93, 0.27, 0.27, 0.22, 0.88, 0.56, 0.64, 0.24)
...
lmer <- lmer(X ~ Exposition+TZP+(1|ID),
             data=H2[H2$ssrt_isoutlier == 0   & H2$ST_Hyposmie == 0, ] ,na.action = na.omit)
tab_model(lmer, show.se = TRUE)
summary(lmer)

#all pvalues of the table S6 analysis (study two Douglas Fir only)
pvaluesH2 <- c(0.003, 0.43, 0.37, 0.59, 0.52, 0.69, 0.63, 0.71, 0.72,
               0.28, .80, 0.34, 0.76, 0.17, 0.22, 0.65, 0.33, 0.43)
...
lmer <- lmer(X ~ Exposition + TZP + (1|ID),
             data=all[all$SANT_con_isoutlier == 0 & all$ST_Hyposmie == 0 
...
#all pvalues of the table S6 analysis (study two Douglas Fir only)
pvaluesALLkorrekt <- c(0.91, 0.41, 0.98, 0.11, 0.35, 0.73, 0.22, 0.31, 0.41,
